...
const actionWords = [
  ...
  { text: "EXPRESS", color: "#3560E2" },
  { text: "CHAT", color: "#3560E2" },
...
const Design = () => {
  ...
  const getRandomWordWithCommonLetter = (currentWordText, excludeColor, excludedChar) => {
    ...
    const emergencyWord = actionWords.find(w => w.text !== currentWordText) || actionWords[0];
  ...
  const changeWord = async () => {
    const currentWordObject = actionWords[currentWordIndexRef.current];
    ...
    if (!nextWordObject) {
      ...
      const newIndex = (currentWordIndexRef.current + 1) % actionWords.length;
    ...
    // Calculated for synchronization with other iframe (something.js).
    ...
    <div className="design-component">
      ...
      <div className="design-wrapper">
        <div className="hero-word-container" ref={containerRef}>
          <span className="word-part" style={{ color: "#E0E3D8" }}>DESIGN</span>
        </div>
      </div>
    </div>
  );
